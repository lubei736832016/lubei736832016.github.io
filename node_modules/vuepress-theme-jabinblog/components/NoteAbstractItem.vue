<template>
    <div :class="classIndex" @click="$router.push(item.path)">
        <!-- https://www.zpzpup.com/assets/image/gd0${imgIndex}.jpg -->
        <div class="draw"><img :src="`https://www.zpzpup.com/assets/image/gd0${item.name}.jpg`" alt=""></div>
...
// 导入的图片
import imgmylist from "../../../imglist.js"
const imgList = imgmylist
const classList = [
    ['abstract-item', 'draw'],
...
export default {
    ...
    // 修改在这里提前修改 再 里面循环 实现改装 图片自定义显示 只需要定义好 name 与 title就可以了
    created() {
        imgList.forEach(element => {
          if(this.item.title==element.title){
            this.item.name = element.name// 修改在这里提前修改 再 里面循环
          }
        }); 
    },
    beforeMount() {
